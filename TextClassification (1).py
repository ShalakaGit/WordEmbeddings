...
#newsgroups_train = fetch_20newsgroups(subset="train",categories=['alt.atheism','talk.religion.misc','rec.sport.baseball'])
newsgroups_train = fetch_20newsgroups(subset="train")
newsgroups_test = fetch_20newsgroups(subset="test",categories=['alt.atheism','talk.religion.misc','rec.sport.baseball',])
...
batch, labels = generate_batch(batch_size=11314, num_skips=2, skip_window=1)
...
with tf.Session(graph=graph) as session:
  ...
  for step in xrange(num_steps):
    ...
    # in the list of returned valuesfor session.run()
...
samples = ['Space','Station','Redesign','PB','SI','Clock','Poll','Car','JSC']
...
for i in range(len(samples)):
    ...
    for k in xrange(top_k):
          close_word = reverse_dictionary[nearest[k]]
          log_str = '%s %s,' % (log_str, close_word)
          print('Closest word is',close_word)
